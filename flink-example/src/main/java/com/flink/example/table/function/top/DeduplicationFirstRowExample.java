...
/**
 * 功能：Deduplication 保留第一行策略
 * 作者：SmartSi
 * CSDN博客：https://smartsi.blog.csdn.net/
 * 公众号：大数据生态
 * 日期：2022/10/18 上午8:20
 */
public class DeduplicationFirstRowExample {
    public static void main(String[] args) {
        ...
        // 设置作业名称
        config.setString("pipeline.name", DeduplicationFirstRowExample.class.getSimpleName());
        ...
        // 创建输出表
        tEnv.executeSql("CREATE TABLE shop_category_order_deduplication (\n" +
                "  category STRING COMMENT '商品类目',\n" +
                "  product_id BIGINT COMMENT '商品Id',\n" +
        ...
        // 执行计算并输出
        tEnv.executeSql("INSERT INTO shop_category_order_deduplication\n" +
                "SELECT\n" +
                "  category, product_id, price, ts_ltz AS `time`, row_num\n" +
                "FROM (\n" +
                "  SELECT\n" +
                "    category, product_id, price, ts_ltz,\n" +
                "    ROW_NUMBER() OVER (PARTITION BY category ORDER BY ts_ltz) AS row_num\n" +
                "  FROM shop_sales\n" +
                ")\n" +
                "WHERE row_num = 1");
    }
